	...
	public function onUpdate($type, $deep) {
		...
		if ($type === Level::BLOCK_UPDATE_NORMAL) {
			...
			if ($decay > 0) {
				...
				if ($this instanceof Lava && $decay < 8 && $k < 8 && $k > 1 && mt_rand(0, 4) !== 0) {
					$k = $decay;
					$flag = false;
				}
				...
				if ($k !== $decay) {
					...
					if ($decay < 0) {
						...
						$this->getLevel()->setBlock($this, Block::get($this->id, $decay), true, true, $deep);
						$this->getLevel()->scheduleUpdate($this, $this->tickRate());
					}
				} elseif ($flag) {
					$this->getLevel()->scheduleUpdate($this, $this->tickRate());
...
//				$this->updateFlow();
			...
			if ($decay >= 0) {
				...
				} elseif ($bottomBlock->canBeFlowedInto() || ($bottomBlock instanceof Liquid && ($bottomBlock->getDamage() & 0x07) !== 0)) {
					$this->getLevel()->setBlock($bottomBlock, Block::get($this->id, $decay | 0x08), true, false, $deep);
					$this->getLevel()->scheduleUpdate($bottomBlock, $this->tickRate());
				} elseif ($decay === 0 || !$bottomBlock->canBeFlowedInto()) {
		...
		for ($j = 0; $j < 4; ++$j) {
			if (
				...
				if (!$blockSide->canBeFlowedInto() && !($blockSide instanceof Liquid)) {
					continue;
				} elseif ($blockSide instanceof Liquid && $blockSide->getDamage() === 0) {
					continue;
				}
