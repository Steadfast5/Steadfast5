...
/**
 * All World related classes are here
 */
...
use pocketmine\event\WorldTimings;
...
use pocketmine\world\format\generic\BaseWorldProvider;
use pocketmine\world\format\generic\EmptyChunkSection;
use pocketmine\world\format\WorldProvider;
use pocketmine\world\generator\LightPopulationTask;
...
use pocketmine\world\generator\GenerationTask;
...
class World implements ChunkManager, Metadatable{
	...
	public static $COMPRESSION_WORLD = 8;
	 ...
	 * @param string $name
	 * @param string $path
	 * @param string $provider Class that extends WorldProvider
	 *
	 * @throws \Exception
	 */
	public function __construct(Server $server, $name, $path, $provider){
		...
		if(is_subclass_of($provider, WorldProvider::class, true)){
			$this->provider = new $provider($this, $path);
			$this->yMask = $provider::getYMask();
			$this->maxY = $provider::getMaxY();
		}else{
			throw new WorldException("Provider is not a subclass of WorldProvider");
		}
		$this->server->getLogger()->info("Preparing world \"" . $this->provider->getName() . "\"");
		...
		if ($this->server->weatherEnabled && $this->getDimension() == self::DIMENSION_NORMAL) {
			WeatherManager::registerWorld($this);
	...
	/**
	 * Unloads the current world from memory safely
	 *
	 * @param bool $force default false, force unload of default world
	 *
	 * @return bool
	 */
	public function unload($force = false){

		$ev = new WorldUnloadEvent($this);

		if($this === $this->server->getDefaultWorld() and $force !== true){
		...
		$this->server->getLogger()->info("Unloading world \"" . $this->getName() . "\"");
		$defaultWorld = $this->server->getDefaultWorld();
		foreach($this->getPlayers() as $player){
			if($this === $defaultWorld or $defaultWorld === null){
				$player->close(TextFormat::YELLOW . $player->getName() . " has left the game", "Forced default world unload");
			}elseif($defaultWorld instanceof World){
		...
		if ($this->weather != null) {
			WeatherManager::unregisterWorld($this);
		}
	...
	public function doTick($currentTick){
		...
		while($this->updateQueue->count() > 0 and $this->updateQueue->current()["priority"] <= $currentTick){
			...
			$block = $this->getBlock($blockData["pos"]);
	...
	public function clearChunkCache(int $chunkX, int $chunkZ){
		unset($this->chunkCache[World::chunkHash($chunkX, $chunkZ)]);
	}
	...
	public function save($force = false){
		...
		$this->server->getPluginManager()->callEvent($ev = new WorldSaveEvent($this));
		if($ev->isCancelled())
			return false;

		$this->provider->setTime((int) $this->time);
		$this->saveChunks();
		if($this->provider instanceof BaseWorldProvider){
			$this->provider->saveWorldData();
		}
	...
	*/
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null) {
		...
		if ($world instanceof World) {
			$above = $world->getBlock(new Vector3($target->x, $target->y + 1, $target->z));
			if ($above instanceof Block) {
				if ($above->getId() === Item::FIRE) {
					$world->setBlock($above, new Air(), true);
				}
			}
		}		
		$target->onBreak($item);
	...
	/**
	 * Returns a list of the Tile entities in this world
	 *
	 * @return Tile[]
	 */
	...
	public function getChunkLoaders(int $chunkX, int $chunkZ){
		return isset($this->chunkLoaders[$index = World::chunkHash($chunkX, $chunkZ)]) ? $this->chunkLoaders[$index] : [];
	}
	...
	public function generateChunkCallback($x, $z, FullChunk $chunk){
		...
		for($xx = -1; $xx <= 1; ++$xx){
			for($zz = -1; $zz <= 1; ++$zz){
				unset($this->chunkPopulationLock[World::chunkHash($x + $xx, $z + $zz)]);
			}
		}
	...
	public function requestChunk($x, $z, $player) {
		...
		$chunkIndex = World::chunkHash($x, $z);
	...
	public function removeEntity(Entity $entity){
		if($entity->getWorld() !== $this){
			throw new WorldException("Invalid Entity world");
		}
	...
	public function addTile(Tile $tile){
		if($tile->getWorld() !== $this){
			throw new WorldException("Invalid Tile world");
		}
	...
	public function generateChunk(int $x, int $z, bool $force = false){
		...
		if(!isset($this->chunkGenerationQueue[$index = World::chunkHash($x, $z)])){
	...
	public function setMetadata($metadataKey, MetadataValue $metadataValue){
		$this->server->getWorldMetadata()->setMetadata($this, $metadataKey, $metadataValue);
	}
	...
	public function removeMetadata($metadataKey, Plugin $plugin){
		$this->server->getWorldMetadata()->removeMetadata($this, $metadataKey, $plugin);
	}
	...
	public function populateChunk(int $x, int $z, bool $force = false){
		...
		if(!$chunk->isPopulated()){
			...
			for($xx = -1; $xx <= 1; ++$xx){
				for($zz = -1; $zz <= 1; ++$zz){
					if(isset($this->chunkPopulationLock[World::chunkHash($x + $xx, $z + $zz)])){
						$populate = false;
						break;
					}
				}
			...
			if($populate){
				if(!isset($this->chunkPopulationQueue[$index])){
					...
					for($xx = -1; $xx <= 1; ++$xx){
						for($zz = -1; $zz <= 1; ++$zz){
							$this->chunkPopulationLock[World::chunkHash($x + $xx, $z + $zz)] = true;
						}
					}
	...
	public function isNight() {
		if ($this->time >= World::TIME_NIGHT && $this->time < World::TIME_SUNRISE) {
			return true;
		}
		return false;
	}
