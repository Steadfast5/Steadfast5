...
use pocketmine\network\protocol\WorldEventPacket;
...
class Weather {
	...
	public function __construct(World $world, $duration = 1200) {
		$this->world = $world;
		$this->weatherNow = self::SUNNY;
		$this->duration = $duration;
		$this->lastUpdate = $world->getServer()->getTick();
	...
	public function calcWeather($currentTick) {
		if ($this->canCalculate()) {
			...
			if ($this->duration <= 0) {
				$duration = mt_rand(
					min($this->world->getServer()->weatherRandomDurationMin, $this->world->getServer()->weatherRandomDurationMax), 
					max($this->world->getServer()->weatherRandomDurationMin, $this->world->getServer()->weatherRandomDurationMax),
				);
			...
			if (($this->weatherNow >= self::RAINY_THUNDER) && ($this->world->getServer()->lightningTime > 0) && is_int($this->duration / $this->world->getServer()->lightningTime)) {
				$players = $this->world->getPlayers();
				if (count($players) > 0) {
					$p = $players[array_rand($players)];
					$x = $p->x + mt_rand(-64, 64);
					$z = $p->z + mt_rand(-64, 64);
					$y = $this->world->getHighestBlockAt($x, $z);
					$this->world->spawnLightning($this->temporalVector->setComponents($x, $y, $z));
				}
			}
		}
	...
	public function setWeather(int $weather, int $duration = 12000) {
		$this->world->getServer()->getPluginManager()->callEvent($ev = new WeatherChangeEvent($this->world, $weather, $duration));
		if (!$ev->isCancelled()) {
