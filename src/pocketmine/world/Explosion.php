...
class Explosion {
				...
				for ($k = 0; $k < $this->rays; ++$k) {
					if ($i === 0 || $i === $mRays || $j === 0 || $j === $mRays || $k === 0 || $k === $mRays) {
						...
						for ($blastForce = $this->size * (mt_rand(700, 1300) / 1000); $blastForce > 0; $blastForce -= $this->stepLen * 0.75) {
							...
							if ($vBlock->y < 0 || $vBlock->y >= $this->world->getMaxY()) {
								break;
							}
							$block = $this->world->getBlock($vBlock);
							if ($block->getId() !== 0 && $block->getId() !== Block::BEDROCK) {
								$blastForce -= ($block->getHardness() / 5 + 0.3) * $this->stepLen;
								if ($blastForce > 0) {
									if (!isset($this->affectedBlocks[$index = World::blockHash($block->x, $block->y, $block->z)])) {
	...
	public function explodeB() {
		...
		$this->world->getServer()->getPluginManager()->callEvent($ev = new EntityExplodeEvent($this->what, $this->source, $this->affectedBlocks, $yield));
		if ($ev->isCancelled()) {
			return false;
		} else {
			$yield = $ev->getYield();
		...
		foreach ($this->affectedBlocks as $block) {
			if ($block->getId() === Block::TNT) {
				...
				$tnt = Entity::createEntity("PrimedTNT", $this->world->getChunk($block->x >> 4, $block->z >> 4), new Compound("", [
					"Pos" => new Enum("Pos", [
				...
				$tnt->spawnToAll();
			} else if (mt_rand(0, 100) < $yield) {
				foreach ($block->getDrops($air) as $drop) {
					$this->world->dropItem($block->add(0.5, 0.5, 0.5), Item::get(...$drop));
				}
			}
			$this->world->setBlock(new Vector3($block->x, $block->y, $block->z), new Air());
		...
		$pk->radius = $this->size;
		$pk->records = $send;
		Server::broadcastPacket($this->world->getUsingChunk($source->x >> 4, $source->z >> 4), $pk);		
		$this->world->addParticle(new HugeExplodeParticle(new Vector3($this->source->x,  $this->source->y, $this->source->z)));	
		...
		$pk1->blockId = -1;
		$pk1->entityType = 1;
		Server::broadcastPacket($this->world->getUsingChunk($source->x >> 4, $source->z >> 4), $pk1);
